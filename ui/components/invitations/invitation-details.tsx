...
export const InvitationDetails = ({ attributes }: InvitationDetailsProps) => {
  ...
  const york_ie = "york.ie";
  const userName = attributes.email.split("@")[0]; // Extracts "vipul"

  const invitationLink = `${baseUrl}/sign-up?invitation_token=${attributes.token}&email=${attributes.email}&company=${york_ie}&user_name=${userName}`;
