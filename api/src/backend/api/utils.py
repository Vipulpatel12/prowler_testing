...
def UserCheck(email,raise_not_found=True) -> validate_invitation:
    
    try:
        # Admin DB connector is used to bypass RLS protection since the invitation belongs to a tenant the user
        # is not a member of yet
        print('heeeeeeeee')
        validate_invitation = Invitation.objects.using(MainRouter.admin_db).filter(
             email=email
        ).exists()
        print('validate_invitation',validate_invitation)

        if validate_invitation ==True :
            print('hello')
    ...
    except validate_invitation.DoesNotExist:
        if raise_not_found:
            raise ValidationError({"invitation_token": "Invalid invitation code."})
        else:
