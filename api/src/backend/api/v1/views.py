...
from api.send_mail import sending_mail
...
from api.utils import CustomOAuth2Client, validate_invitation,UserCheck
...
class UserViewSet(BaseUserViewset):
    ...
    def create(self, request, *args, **kwargs):
        ...
        print('invitation_token>>>',request.data)

        user_check=UserCheck(email=request.data['email'])
        print('user_check>',user_check)
        print('request.data',request.data['email'])
        if user_check==False and request.data['email'] !='vipulp@york.ie':
            print('done')
            return  Response(
                        data={"detail": "You are not authorized to perform this action."},
                        status=status.HTTP_403_FORBIDDEN
                    )
        print('user_checkuser_checkuser_checkuser_check>>>',user_check)
